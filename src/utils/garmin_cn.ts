...
export const getGaminCNClient = async (): Promise<GarminClientType> => {
    ...
    const GCClient = new GarminConnect({ username: GARMIN_USERNAME, password: GARMIN_PASSWORD });
    ...
    try {
        ...
        if (!emailAddress) {
            throw Error('佳明中国区登录失败，请检查填入的账号密码或您的网络环境')
        }
...
export const migrateGarminCN2GarminGlobal = async (count = 200) => {
    ...
    const { getGaminGlobalClient } = require('./garmin_global');
...
export const syncGarminCN2GarminGlobal = async () => {
    ...
    if (latestGlobalActStartTime === latestCnActStartTime) {
